...
class rdtClient {
	public:
		...
		unsigned int length;
		int sock, state, lastFinal, lastStart, lastSize;
		char lastSentMsg[BUFFER_SIZE];
		float prob_corruption;
...
void rdtClient::udtRcv()	{
	...
	int n = recvfrom(sock, (PACKET*)ackPacket, BUFFER_SIZE, 0, (struct sockaddr *)&from, &length);
	if(n < 0)	{
...
{	
	...
	if(argc < 6) {
        cerr << "Usage: " << argv[0] << " [host_ip] [port_number] [input_file_name] [output_file_name] [probability_of_corruption]" << endl;
        exit(0);
    }
